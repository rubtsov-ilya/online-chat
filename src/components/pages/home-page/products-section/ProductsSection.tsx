...
const productsSection: FC = () => {
  ...
  const { data: userData = null, isLoading: isUserDateLoading } =
    useGetUserStateQuery({ uMockid: uMockid! }, { skip: uMockid === null });
          ...
          <div className={styles['products-section__grid']}>
            ...
            {errorStatus === 404 && (
              <p className={styles['products-section__error']}>
                Não há nada para "{searchParameter}"
              </p>
            )}
            {/* cards */}
            {!errorStatus &&
              ...
              products.map((product) => {
                return (
                  <ProductCard
                    isUserDateLoading={isUserDateLoading}
                    uMockid={uMockid}
                    key={product.id}
                    product={product}
                    cart={userData?.cart}
